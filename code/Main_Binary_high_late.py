...
# Define paths
name = 'Binary_high_late'

dir = '/Users/jeromechoi/Documents/jerome/Documents/WISC/BMI/Daifeng Wang/Cooperative learning/Coop_DOT/Results/Simluated/new_111624/Binary_high_late_test/'
output = '/Users/jeromechoi/Documents/jerome/Documents/WISC/BMI/Daifeng Wang/Cooperative learning/Coop_DOT/Results/Simluated/new_111624/Binary_high_late_test/'

# dir = '/project8/Dissertation_JC/Coop_DOT/Simulation/111624/data/'
# output = '/project8/Dissertation_JC/Coop_DOT/Simulation/111624/Binary_high_early/'

data1_path = dir + 'binary_high_late_x1.csv'
data2_path =  dir + 'binary_high_late_x2.csv'
...
def load_and_prepare_data(batch_size, output, name, data1_path, data2_path):
    ...
    data1 = pd.read_csv(data1_path, header=0, index_col=0, sep=',', encoding='utf-8')
    data2 = pd.read_csv(data2_path, header=0, index_col=0,sep=',', encoding='utf-8')
...
class Model(nn.Module):
    def __init__(self, *input_dims, dim, dropout):
        ...
        self.layers = nn.ModuleList([
            nn.Sequential(
                ...
                # nn.BatchNorm1d(int(input_dim*0.25)),
                # nn.Dropout(dropout),
                # nn.LeakyReLU(),

                # nn.Linear(input_dim*4, int(input_dim*5)),
                # nn.BatchNorm1d(int(input_dim*5)),
                # nn.Dropout(dropout),
                # nn.LeakyReLU(),

                # nn.Linear(input_dim*5, int(input_dim*4)),
                # nn.BatchNorm1d(int(input_dim*4)),
                # nn.Dropout(dropout),
                # nn.LeakyReLU(),

                # nn.Linear(int(input_dim*1.4), int(input_dim*1.3)),
                # nn.BatchNorm1d(int(input_dim*1.3)),
                # nn.Dropout(dropout),
                # nn.LeakyReLU(),

                # nn.Linear(int(input_dim*1.3), int(input_dim*1.2)),
                # nn.BatchNorm1d(int(input_dim*1.2)),
                # nn.Dropout(dropout),
                # nn.LeakyReLU(),
    ...
    def forward(self, *X):
        ...
        # [(z1, mu1, ls1), (z2, mu2, ls2)], [log_out1, log_out2]
...
def train_model(config, fold_number, max_epochs, early_stopper, device, output, name, train_loader_A, train_loader_B,
    ...
    best_auroc_overall =  0.0
    best_auroc_overall =  -np.inf
    ...
    for fold, (pair_A, pair_B) in enumerate(zip(splits.split(data_A), splits.split(data_B))):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=learning_gamma, patience=10, verbose=True)
        ...
        for epoch in range(epochs):
            ...
            fpr, tpr, thresholds = roc_curve(actual_values, predicted_probs)

            # Calculate TPR (True Positive Rate), FPR (False Positive Rate), and select threshold
            tpr = tpr[1:]
            fpr = fpr[1:]

            # Calculate metrics such as Youden's J statistic or other specific thresholds
            # For AUROC, the optimal threshold is often related to maximizing the true positive rate
            optimal_threshold_index = np.argmax(tpr - fpr)
            optimal_threshold = thresholds[optimal_threshold_index]

            print(f"AUROC (val) optimal threshold: {optimal_threshold}")

            # Initialize AUROC metric
            auroc_metric = AUROC(task='binary')

            # Convert numpy arrays back to PyTorch tensors for binary classification
            predicted_probs_tensor = torch.tensor(predicted_probs, dtype=torch.float32)
            ...
            predicted_values_binary = (predicted_probs_tensor >= optimal_threshold).float()

            # Update and compute AUROC
            try:
                auroc_metric.update(predicted_probs_tensor, actual_values_tensor)
                auroc = auroc_metric.compute()
                print(f"AUROC (from AUROC (val) metric): {auroc:.4f}")
            except Exception as e:
                print(f"Error computing AUROC (val): {e}")
                auroc = 0.0

            # Append AUROC value to history
            history['AUROC_val'].append(auroc.item())

            print(f"AUROC_val (val): {auroc:.4f}")

            # Calculate AUPRC
            precision, recall, _ = precision_recall_curve(actual_values, predicted_probs)
            ...
            # Append AUPRC value to history
            history['AUPRC_val'].append(auprc_score)

            print(f"AUPRC_val (val): {auprc_score:.4f}")

            # Compute Precision-Recall curve
            precision, recall, pr_thresholds = precision_recall_curve(actual_values, predicted_probs)

            # Calculate F1 scores for all Precision-Recall thresholds
            f1_scores = 2 * (precision * recall) / (precision + recall)
            f1_scores[np.isnan(f1_scores)] = 0  # Handle any NaNs in F1 scores

            # Find the best F1 score and corresponding threshold
            best_f1_index = np.argmax(f1_scores)
            best_f1_threshold = pr_thresholds[best_f1_index] if best_f1_index < len(pr_thresholds) else 1.0
            best_f1_score = f1_scores[best_f1_index]

            print(f"Best F1 Score (val): {best_f1_score:.4f}")
            print(f"Best F1 Score (val) Threshold: {best_f1_threshold:.4f}")

            # Convert predicted probabilities to binary using the best F1 threshold
            predicted_values_binary = (predicted_probs >= best_f1_threshold).astype(float)

            # Calculate accuracy with the best F1 threshold
            accuracy = np.mean(predicted_values_binary == actual_values)
            print(f"Accuracy (val) with best F1 threshold: {accuracy:.4f}")

            history['accuracy_val'].append(accuracy)
            print(f'accuracy_val: {accuracy}')
        ...
        with torch.no_grad():
            ...
            holdout_KLD_loss_A, holdout_KLD_loss_B, holdout_OT_loss, holdout_classification_loss, predicted_values, actual_values, accuracy = evaluate_holdout(model, holdout_loader_A, holdout_loader_B, nn.BCEWithLogitsLoss(), device)
            ...
            holdout_history['accuracy_eval'].append(accuracy)
            ...
            # Calculate TPR (True Positive Rate), FPR (False Positive Rate), and select threshold
            tpr = tpr[1:]  # Exclude the first point (0, 0)
            fpr = fpr[1:]  # Exclude the first point (0, 0)
            ...
            print(f"AUROC optimal threshold: {optimal_threshold}")
            ...
            try:
                auroc_metric.update(predicted_probs_tensor, actual_values_tensor)
                auroc = auroc_metric.compute()
                print(f"AUROC (from AUROC metric): {auroc:.4f}")
            except Exception as e:
                print(f"Error computing AUROC: {e}")
                auroc = 0.0  # Handle the case where AUROC computation fails

            # Append AUROC value to history
            holdout_history['AUROC_eval'].append(auroc.item())

            print(f"AUROC (eval): {auroc:.4f}")
            ...
            # Append AUPRC value to history
            holdout_history['AUPRC_eval'].append(auprc_score)

            print(f"AUPRC_eval (eval): {auprc_score:.4f}")

            all_models.append(model.state_dict())

    # End of fold loop

    return (model, history, holdout_history, predicted_probs, actual_values, {"all_models": all_models})
