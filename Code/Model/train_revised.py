...
def train_model_binary(model, data1_path, data2_path, batch_size, learning_rate, m_type, epochs, save_path, splits, device, **kwargs):
    ...
    for fold, (pair_A, pair_B) in enumerate(zip(splits.split(train_loader_A.dataset), splits.split(train_loader_B.dataset))):
        ...
        train_loader_A_fold = DataLoader(torch.utils.data.Subset(train_loader_A.dataset, train_idx_A), batch_size=batch_size, shuffle=True)
        val_loader_A_fold = DataLoader(torch.utils.data.Subset(val_loader_A.dataset, val_idx_A), batch_size=batch_size)
        
        train_loader_B_fold = DataLoader(torch.utils.data.Subset(train_loader_B.dataset, train_idx_B), batch_size=batch_size, shuffle=True)
        val_loader_B_fold = DataLoader(torch.utils.data.Subset(val_loader_B.dataset, val_idx_B), batch_size=batch_size)
